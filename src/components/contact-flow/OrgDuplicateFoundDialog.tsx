...
import { Warning, X, Sparkle, CheckCircle, Database } from '@phosphor-icons/react';
...
interface Props {
  open: boolean;
  candidate: OrgDuplicateCandidate | null;
...
const FIELDS: { key: keyof NewCompanyData | keyof OrgDuplicateCandidate; label: string }[] = [
...
export function OrgDuplicateFoundDialog({ open, candidate, newCompany, onKeepExisting, onSmartMerge, onCreateNew, onClose }: Props) {
      ...
      <div className="relative bg-[var(--surface-card)] border border-[var(--border)] rounded-xl shadow-lg w-full max-w-[900px] max-h-[90vh] overflow-y-auto animate-[fadeUp_0.2s_ease-out]">
        ...
        <div className="grid grid-cols-2 gap-0 p-5">
          ...
          <div className="border-r border-[var(--border-subtle)] pr-5">
            <div className="text-[10px] font-bold text-[var(--text-tertiary)] uppercase tracking-wider mb-3 flex items-center gap-1.5">
              Existing Company
              <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[9px] font-bold bg-[var(--brand-bg)] text-[var(--brand-primary)] border border-[var(--brand-primary)]">
                <Database size={9} weight="fill" /> Roadrunner CRM
              </span>
            </div>
            <div className="flex items-center gap-2.5 mb-4">
              <div
                className="w-10 h-10 rounded-[6px] flex items-center justify-center text-[13px] font-extrabold text-white"
              ...
              <div>
                ...
                <div className="text-[11px] text-[var(--text-tertiary)]">{candidate.industry}</div>
              </div>
            </div>

            {FIELDS.map((f) => {
              const val = (candidate as any)[f.key] as string;
              const newVal = (newCompany as any)[f.key] as string;
          ...
          <div className="pl-5">
            ...
            <div className="flex items-center gap-2.5 mb-4">
              <div className="w-10 h-10 rounded-[6px] flex items-center justify-center text-[13px] font-extrabold text-white bg-[var(--brand-primary)]">
                {initials(newCompany.name || '?')}
              </div>
              <div>
                <div className="text-[14px] font-bold text-[var(--text-primary)]">{newCompany.name || 'New Company'}</div>
                <div className="text-[11px] text-[var(--text-tertiary)]">{newCompany.industry || '—'}</div>
              </div>
            </div>

            {FIELDS.map((f) => {
              const val = (newCompany as any)[f.key] as string;
